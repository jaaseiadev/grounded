...
It is a real retrieval-augmented generation system: documents are extracted, chunked, embedded with OpenAI, indexed in Supabase PostgreSQL with pgvector, retrieved by semantic similarity, and passed to a schema-constrained model response. Retrieval and citations are never simulated.
...
- Batched OpenAI embeddings stored in a 1,536-dimensional pgvector column
...
- OpenAI structured JSON-schema output plus Zod validation
...
- Backend DTO validation, global errors, security headers, CORS, typed Supabase access, and provider-isolated OpenAI integration
...
flowchart TD
  ...
  API -->|embeddings + structured generation| OpenAI[OpenAI API]
  Vector -->|top scoped chunks| API
  API -->|validated citations| Browser
```

Angular owns presentation and browser interaction only. NestJS owns credentials, upload validation, extraction, chunking, embeddings, retrieval, prompt construction, structured generation, citation validation, and persistence. The browser never receives an OpenAI key or Supabase service role.
...
4. OpenAI generates embeddings in batches.
...
8. OpenAI streams a strict structured response.
...
| Backend   | NestJS 11, Node.js 22, TypeScript, OpenAI SDK 7, Supabase JS 2, Zod, class-validator                             |
...
NestJS looks for both `backend/.env` and the repository-root `.env`.
...
| `OPENAI_API_KEY`            | Yes      | Backend-only OpenAI credential                                  |
| `OPENAI_CHAT_MODEL`         | Yes      | Structured chat model, for example `gpt-5-mini`                 |
| `OPENAI_EMBEDDING_MODEL`    | Yes      | Must produce 1,536 dimensions; default `text-embedding-3-small` |
...
Backend tests mock provider behavior and require no OpenAI key. They cover upload validation, sanitization, chunking, retrieval scope, prompt construction, structured response validation, progressive answer extraction, and citation allow-listing. Frontend tests cover application routing, signal-backed document loading, upload progress, and document scope output.
...
- Never place the OpenAI key or Supabase service role in Angular.
