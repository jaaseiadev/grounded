...
5. Chunks are embedded in batches of 50 through OpenRouter using a configured 1,536-dimensional embedding model.
...
Retrieved chunks are serialized into tagged blocks containing the authoritative chunk ID, source name, page, section, and exact text. OpenRouter streams a reasoning-enabled, strict JSON-schema response. The backend extracts the partial `answer` string for progressive UI delivery while retaining the complete JSON for final Zod validation and the aggregate reasoning-token count for diagnostics.
...
The Angular client owns an `AbortController`. Stopping generation closes the request, the NestJS controller aborts the OpenRouter request, and a useful partial answer is retained in conversation history when available.
